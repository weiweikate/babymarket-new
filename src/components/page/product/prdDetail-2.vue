  ...
  <div class="detailPage">
    ...
    <div class="typeDiv">
      <yd-popup v-model="show" position="bottom" height="5rem">
        <div class="typeInfo">
          ￥ <span class="price">{{specificationData ? specificationData.levelPrice : prdInfos.SalePrice}}</span>
          (库存{{specificationData ? specificationData.Inv : prdInfos.Inv}}件)
        ...
        <div class="container">
          <div class="specification"  v-for="(specification,index) in SpecificationKey" :key="index">
            ...
            <ul>
              <li v-for="type, key in specification.S_Value_Detail" :key="key" @click="choose(index,key)" :class="typeIndex[index]=== key? 'active': ''">{{type.S_Value_Name}}</li>
            </ul>
          </div>
          <div class="buyNum">
            <span>数量</span>
...
//        })
      ...
      choose (index0,key0) {
        let self = this
        this.$set(this.typeIndex,index0,key0)
        this.number = 1
        // 规格所在的组别
        let selectGroupData = this.SpecificationKey[index0]
        // 规格所在的组别的所有规格
        let selectGroupItemArray = selectGroupData.S_Value_Detail
        // 选择的一个规格
        let selectGroupItem = selectGroupItemArray[key0]
        if(parseInt(selectGroupItem.Inv) == 0){
          sValueDetail.selected = false
          return
        }
        //2维组合（以下代码的目的：找到未被选中的组，刷新他们的库存信息，为0时，变成虚线，并不可点击)
        if(this.SpecificationKey.length == 2){
          this.SpecificationKey.forEach((eachGroupDataInLoop)=>{
            if (eachGroupDataInLoop.Id !== selectGroupData.Id){
              ...
              otherGroupItemArray.forEach((otherGroupItem)=>{
                self.searchSepecificationIn(self.allSepcificationArray,[selectGroupItem,otherGroupItem])
              })
            }
          })
        }
        ...
        let selectedArray = []
        this.SpecificationKey.forEach((data)=>{
        ...
        if (this.SpecificationKey.length == this.selectedArray.length) {
          let specificationData = this.searchSepecificationIn(this.allSepcificationArray,this.selectedArray)
      ...
      searchSepecificationIn(allSpecificationArr,groupItemArr){
        ...
        allSpecificationArr.forEach((specification) => {
          if (groupItemArr.length == 2) {
            if (specification.SpecificationItem1Id && specification.SpecificationItem2Id ) {
            ...
            if (specification.SpecificationItem1Id) {
              let specificationItemId = "SpecificationItem"+ selectGroupItem.SpecificationKey +"Id"
              let isSelectGroupItemEqual = specification[specificationItemId] === selectGroupItem.Id
      ...
      addShoppingCarts () {
        ...
        this.axios.post(url,{"Items":[{"AddOperationId": _addShoppingCarts,"Data":{"EntityName":"Shopping_Cart","Items":{"CreatorId":this.isLogin.userId,"MemberId":this.isLogin.userId,"Price":this.myPrice,"ProductId":this.prdId,"Product_SId":this.specificationData.Id,"Qnty":String(this.number),"isSelect":false},"Status":"New"}}]})
            .then((res) => {
          if(JSON.stringify(res.data) == "{}"){
      ...
      newOrder () {
        let url = this.writeUrl
        if (this.SpecificationKey){
          if(!this.specificationData || !this.specificationData.Id){
            this.$dialog.toast({mes: '请选择产品规格', timeout: 1500})
            return
          }
        }
      ...
      newOrderDetails () {
        let url = this.writeUrl
        this.axios.post(url,{"Items":[{"AddOperationId": _newOrderDetails,"Data":{"EntityName":"Order_Line","Items":{"ImgId":this.prdInfos.ImgId,"OrderId": this.orderId,"Price": this.myPrice,"ProductId":this.prdId,"Product_SId":this.specificationData.Id,"Product_ShowName": this.prdInfos.ShowName,"Qnty":String(this.number),"S_Name":this.specificationData.S_FullName},"Status":"New"}}]})
            .then((res) => {
          console.log(res.data)
          if(JSON.stringify(res.data) == "{}"){
            this.$dialog.loading.close()
            window.sessionStorage.buyPrd = JSON.stringify([{prdName:this.prdInfos.ShowName,img:this.prdInfos.ImgId,price:this.myPrice,type:this.specificationData.S_FullName,pcs:this.number,WarehouseId:this.prdInfos.StoreId}])
